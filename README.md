...
![1767713581016](image/1767713581016.png)
...
### 2.5 AI Image Tagging

When adding new paths, if auto-tagging is enabled, the system will automatically tag images using AI models during scanning for better organization.

* Current plugin supports **WD14 Tagger (ONNX version)**
* [Download Link](https://huggingface.co/SmilingWolf/wd-v1-4-vit-tagger-v2/tree/main)
* Required files:
  - `model.onnx`: Core inference file (~300-500MB)
  - `selected_tags.csv`: Tag mapping file containing names, NSFW classification, and weights
...
- **AI Tagging not working?** Check if the feature is enabled in settings and verify the model files are correctly placed in `plugins/ai-tagger`.
- **Development Conventions**:
  - **Modularity Principle**: If a code file exceeds **500 lines**, it must be split into smaller components or modules.

### 4. Function preview

- local models management desk
