...
新增本地路径的时候，若开启了自动达标，在扫描的过程中，会使用AI模型对图片进行预处理达标，用于后期分类管理

* 当前插件适配的是 **WD14 Tagger（ONNX 版本）**
* [下载地址](https://huggingface.co/SmilingWolf/wd-v1-4-vit-tagger-v2/tree/main)
* `model.onnx`：WD14 Tagger 的 ONNX 格式模型文件（核心推理文件，大小约 300-500MB）；
* `selected_tags.csv`：标签映射文件（包含标签名称、NSFW 分类、权重，插件通过此文件解析打标结果）。
...
- 智能达标的功能不生效？ 检查设置是否开启和放置的识别模型是否正常放置到 `plugins/ai-tagger`
